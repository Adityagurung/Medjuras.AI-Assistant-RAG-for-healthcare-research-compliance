...
from llm.embeddings import embed_texts, resolve_embedding_model
...
def _resolve_models():
    chat = get_chat_model() if use_mygenassist() else os.getenv("OPENAI_DEFAULT_MODEL", "gpt-4o-mini")
...
def _cosine_from_vectors(v1, v2):
...
def _cosine_sim_api(a, b):
    if not a or not b:
        return 0.0
    vecs = embed_texts([a, b])
    v1 = np.array(vecs[0], dtype=np.float32)
    v2 = np.array(vecs[1], dtype=np.float32)
    return _cosine_from_vectors(v1, v2)


def _cosine_sim_local(embedder, a, b):
    if not a or not b:
        return 0.0
    v1 = np.asarray(embedder.encode(a))
    v2 = np.asarray(embedder.encode(b))
    return _cosine_from_vectors(v1, v2)
...
def run_model_comparison(
    ...
    if embedding_backend == "local":
        ...
        st_name = embedding_model_name or "multi-qa-MiniLM-L6-cos-v1"
        embedder = SentenceTransformer(st_name)
        resolved_name = st_name
        score_fn = lambda ans, gt: _cosine_sim_local(embedder, ans, gt)
    else:
        resolved_name = resolve_embedding_model(embedding_model_name)
        score_fn = _cosine_sim_api
    ...
    result = {"n_questions": len(per_q), "local": local, "embedding_backend": embedding_backend, "embedding_model": resolved_name, "summary": summary, "scores_by_model": scores_by_model, "per_question": per_q}
...
if __name__ == "__main__":
    out = run_model_comparison(max_samples=5, local=True, embedding_backend="api")

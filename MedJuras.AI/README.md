...
### **1. MedRAG Corpus**
A curated medical corpus from the [MedRAG dataset](https://huggingface.co/datasets/MedRAG/textbooks):
...
### **2. EU Regulatory Documents**
- **Clinical Trials Regulation (CTR)**: EU clinical trial transparency requirements
- **Antimicrobial Resistance (AMR)**: ECDC guidelines and policy documents
- **Medical Device Regulation (MDR)**: EU compliance frameworks
...
1. **Download**: Automated retrieval via Notebook 01 (HuggingFace MedRAG + EU PDFs)
2. **PDF Processing**: Docling-based conversion to markdown
3. **Normalization**: medrag_process.py creates unified records.json
4. **Chunking**: LangChain RecursiveCharacterTextSplitter with jurisdiction metadata
5. **Indexing**: Dual-index to Elasticsearch and Qdrant with hybrid vectors
...
### 1. **Data Ingestion** 
```
MedRAG JSON + EU PDFs → Docling Conversion → Normalized Records → Chunking → JSONL
```
- Download MedRAG from HuggingFace (textbooks, PubMed)
- Fetch EU regulatory PDFs (CTR, AMR documents)
- Docling converts PDFs to clean markdown
...
│   │   ├── llm_judge.py
│   │   └── ground_truth.py
│   ├── guards/              # Safety and scope guards
│   │   └── scope_guard.py
...
│   │   ├── query_rewriter.py
│   │   └── rag_utils.py
│   ├── monitoring/          # Feedback and database
│   │   ├── database.py
│   │   └── feedback.py
│   ├── search/              # Retrieval implementations
│   │   ├── hybrid_search.py
│   │   ├── es_search.py
│   │   └── qdrant_search.py
...
├── data/
│   ├── raw/                 # Raw data files
│   │   ├── medrag/          # MedRAG JSON files
│   │   └── pdfs/            # EU regulatory PDFs
...
│   ├── 01_download_medrag_and_pdfs_data.ipynb
│   ├── 02_pdf_to_markdown_marker.ipynb
│   ├── 03_chunk_and_jurisdiction_metadata.ipynb
│   ├── 04_ground_truth_generation.ipynb
│   ├── 05_index_to_qdrant_hybrid.ipynb
│   ├── 06_retrieval_evaluation.ipynb
│   ├── 07_rag_evaluation_llm_judge.ipynb
│   └── 08_agent_demo.ipynb
├── grafana/
